...
app = FastAPI(title="TradeBot AI Long/Short Trader", version="1.1.0", lifespan=lifespan)
...
@app.get("/")
def root():
    return {"name": "TradeBot AI Long/Short Trader", "version": "1.1.0", "paper_trading": settings.paper_trading, "ui": "/index.html", "data": {"directory": settings.data_dir, "database": str(store.db_path), "collector_interval_seconds": settings.collector_interval_seconds}, "modes": {"SCALP": {"timeframe": settings.scalping_timeframe, "min_confidence": settings.scalp_min_confidence, "max_positions": settings.max_scalp_positions, "model_ready": engine.ready("SCALP"), "analysis": ["1m", "5m", "15m", "1h", "4h"]}, "SWING": {"timeframe": settings.swing_timeframe, "min_confidence": settings.swing_min_confidence, "max_positions": settings.max_swing_positions, "model_ready": engine.ready("SWING"), "analysis": ["5m", "15m", "1h", "4h"]}}, "demo": demo.status(), "sides": ["LONG", "SHORT"], "time_based_exit": False, "dynamic_exit": True}


@app.get("/health")
def health():
    return {"status": "ok", "paper_trading": settings.paper_trading, "collector_running": collector.running, "position_monitor": monitor_task is not None, "demo_running": demo.enabled, "database": str(store.db_path)}
...
async def collect_now():
    scalp = await collector.collect_mode("SCALP"); swing = await collector.collect_mode("SWING")
...
def demo_trades(limit: int = 100):
    return {"items": demo.trades(limit)}
...
def demo_toggle(payload: DemoToggle):
    ...
    if payload.enabled:
        ...
        demo_task.cancel(); demo_task = None
    return result
    ...
    for symbol in settings.symbol_list:
        try:
            ...
            for setup in setups:
                ...
                if persist: store.save_analysis(setup)
        except Exception:
            errors += 1
...
async def scan_all(mode: str = "SCALP"):
    return await _scan_mode(mode, persist=True)


@app.get("/scan/all")
async def scan_all_modes():
    scalp, swing = await asyncio.gather(_scan_mode("SCALP", persist=True), _scan_mode("SWING", persist=True))
    all_setups = scalp["setups"] + swing["setups"]; all_setups.sort(key=lambda x: (x.probability, x.expected_value_r), reverse=True)
...
async def auto_paper_trade(mode: str, side: str, symbol: str):
    ...
    _, contexts, df = await analyze_symbol(symbol, mode); scorer = score_long_setup if side == "LONG" else score_short_setup
    ...
    position = broker.open_position(setup, decision.quantity); store.save_analysis(setup)
