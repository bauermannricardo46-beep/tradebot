...
def demo_config(payload:DemoConfig):
    ...
    try: return demo.configure(payload.budget,payload.risk_per_trade,payload.max_positions)
...
def demo_toggle(payload:DemoToggle):
    ...
    if payload.enabled:
        if demo_task is None or demo_task.done(): demo_task=asyncio.create_task(demo_loop())
    elif demo_task:
...
def demo_reset():
    global demo_task
    if demo_task: demo_task.cancel(); demo_task=None
    return demo.reset()
...
async def market_overview():
    ...
    for symbol in settings.symbol_list:
        ...
        except Exception as exc: items.append({"symbol":symbol,"error":str(exc)})
    return {"items":items}
...
@app.get("/scan")
async def scan_all(mode:str="SCALP"): return await _scan_mode(mode,True)
...
@app.get("/positions")
def positions(): return {"equity":broker.equity,"realized_pnl":broker.realized_pnl,"positions":broker.open_positions()}
