...
class DemoEngine:
    ...
    def configure(self, budget: float, risk_per_trade: float = 0.005, max_positions: int = 5) -> dict[str, Any]:
        ...
        with self.lock, self._connect() as conn:
            ...
            conn.execute("UPDATE demo_account SET starting_budget=?,equity=?,risk_per_trade=?,max_positions=?,updated_at=? WHERE id=1", (budget,budget,risk_per_trade,max_positions,self._now()))
        return self.status()
    ...
    def reset(self) -> dict[str, Any]:
        with self.lock, self._connect() as conn:
            budget = float(conn.execute("SELECT starting_budget FROM demo_account WHERE id=1").fetchone()[0])
    ...
    def consider_setup(self, setup: Any) -> bool:
        with self.lock, self._connect() as conn:
            ...
            if not account["enabled"]: return False
            if conn.execute("SELECT COUNT(*) FROM demo_positions WHERE status='OPEN'").fetchone()[0] >= account["max_positions"]: return False
            if conn.execute("SELECT 1 FROM demo_positions WHERE status='OPEN' AND symbol=? AND side=? AND mode=?", (setup.symbol,setup.side,setup.mode)).fetchone(): return False
    ...
    async def update_positions(self, fetch_klines) -> int:
        ...
        for p in positions:
            try:
                ...
                if not df.empty:
                    c=df.iloc[-1]; self._apply_candle(p,float(c.high),float(c.low)); changed += 1
    ...
    def _apply_candle(self,p,high:float,low:float) -> None:
        with self.lock, self._connect() as conn:
            ...
            if not row or row["status"]!="OPEN": return
            side=row["side"]; tp1_hit=bool(row["tp1_hit"]); stop=float(row["stop_loss"]); trailing=row["trailing_stop"]
            trail=float(row["trail_distance"]); entry=float(row["entry"]); exit_price=None; reason=None
            if side=="LONG":
            ...
            if exit_price is None:
                conn.execute("UPDATE demo_positions SET stop_loss=?,trailing_stop=?,tp1_hit=? WHERE id=?",(stop,trailing,int(tp1_hit),row["id"])); return
            qty=float(row["quantity"]); pnl=((exit_price-entry) if side=="LONG" else (entry-exit_price))*qty
            risk=abs(entry-stop); signed_r=((exit_price-entry) if side=="LONG" else (entry-exit_price))/risk if risk else 0.0
            result="WIN" if pnl>0 else "LOSS"; now=self._now()
    ...
    def status(self) -> dict[str, Any]:
        with self.lock, self._connect() as conn:
            a=self._account(conn); total=conn.execute("SELECT COUNT(*) FROM demo_trades").fetchone()[0]; wins=conn.execute("SELECT COUNT(*) FROM demo_trades WHERE result='WIN'").fetchone()[0]
            pnl=float(a["equity"])-float(a["starting_budget"])
            return {"enabled":bool(a["enabled"]),"budget":float(a["starting_budget"]),"equity":float(a["equity"]),"pnl":pnl,"pnl_pct":pnl/float(a["starting_budget"])*100 if a["starting_budget"] else 0.0,"risk_per_trade":float(a["risk_per_trade"]),"max_positions":int(a["max_positions"]),"open_positions":int(conn.execute("SELECT COUNT(*) FROM demo_positions WHERE status='OPEN'").fetchone()[0]),"closed_trades":total,"wins":wins,"losses":total-wins,"win_rate":wins/total*100 if total else 0.0}
    ...
    def trades(self,limit:int=100)->list[dict[str,Any]]:
        with self.lock,self._connect() as conn: return [dict(r) for r in conn.execute("SELECT * FROM demo_trades ORDER BY id DESC LIMIT ?",(max(1,min(limit,500)),)).fetchall()]
